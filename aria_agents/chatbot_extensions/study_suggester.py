...
def create_pubmed_query_function(
    ...
    async def query_pubmed(
        ...
        await call_agent(
            ...
            tools=[
                check_pmc_query_hits,
                create_corpus_function(artifact_manager, config),
            ],
        )
...
def create_create_diagram_function(
    ...
    @schema_tool
    async def create_diagram() -> Dict[str, str]:
        ...
        suggested_study_content = await get_file(
            "suggested_study.json", artifact_manager
        )
...
def create_summary_website_function(
    ...
    async def create_summary_website() -> Dict[str, str]:
        """IF THE USER ASKS YOU TO GENERATE A SUMMARY WEBSITE, FIRST USE THE
        `create_diagram`/`StudyDiagram` TOOL TO CREATE A DIAGRAM THAT CAN BE USED FOR THIS FUNCTION.
        Create a summary website for the suggested study."""
