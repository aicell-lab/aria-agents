...
from aria_agents.utils import SchemaToolReturn, ArtifactFile, is_artifacts_available, StatusCode
...
async def add_to_corpus(
    ...
    if not is_artifacts_available():
        error_msg = f"Cannot add file '{file_name}' to corpus: No artifact manager available"
        return SchemaToolReturn(
            response=error_msg,
            status=StatusCode.bad_request(error_msg)
        )
    ...
    return SchemaToolReturn(
        ...
        to_save=[artifact],
        status=StatusCode.created(success_msg)
    )