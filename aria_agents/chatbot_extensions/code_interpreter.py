...
def create_analyzer_function(data_store: HyphaDataStore = None) -> Callable:
    ...
    async def run_code_interpreter(
        ...
        codes_fname = os.path.join(out_dir, "codes.json")
        
        # Generate the file descriptions
        # files = ["/Users/gkreder/org/data/CC/4918D6-79F7-49FA-B79C-554D6A9452EC/sample_data/MTBLS3/a_live_mtbl3_metabolite profiling_mass spectrometry.txt","/Users/gkreder/org/data/CC/4918D6-79F7-49FA-B79C-554D6A9452EC/sample_data/MTBLS3/i_Investigation.txt","/Users/gkreder/org/data/CC/4918D6-79F7-49FA-B79C-554D6A9452EC/sample_data/MTBLS3/m_live_mtbl3_metabolite profiling_mass spectrometry_v2_maf.tsv","/Users/gkreder/org/data/CC/4918D6-79F7-49FA-B79C-554D6A9452EC/sample_data/MTBLS3/s_live_mtbl3.txt"]
        # file_explorer = Role(
        #     name = "File Explorer",
        ...
        code_generator = Role(
            ...
            instructions = "You are a hypothesis code generator. Based on the hypotheses generated, generate Python code to test the hypotheses on the data generated from a scientific study. The result of the code should be a visualization of the results of the hypothesis test in a shown matplotlib plot. In addition to the visualization, the plot should include a text box with the hypothesis, procedure, and results summary of the hypothesis test.",
            constraints = None,
            register_default_events = True,
            model = CONFIG['llm_model']
        )
        test_codes = [code_generator.acall
                      ([
                          "Take the following hypothesis and generate Python code to test the hypothesis on the data generated from a scientific study.",
                          "Use the `run_code` tool to explore the files to make sure you are referring to the correct names/keys/values in the data and parsing it correctly.",
                          f"The files are available in the directory: {data_dir}",
                          hypothesis], 
                          output_schema=HypothesisCode,
                          tools=[run_code])
                        for hypothesis in hypotheses.hypotheses[0 : 2]]
        test_codes = await asyncio.gather(*test_codes)
        hypothesis_codes = HypothesisCodes(codes=test_codes)
        with open(codes_fname, 'w') as f:
            json.dump(hypothesis_codes.dict(), f, indent=4)
        ipdb.set_trace()
