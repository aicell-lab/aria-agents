...
from aria_agents.chatbot_extensions.study_suggester import (
    ...
    create_create_diagram_function,
)
...
@pytest.mark.asyncio
@patch(
    "aria_agents.chatbot_extensions.study_suggester.get_query_index_dir",
    return_value="/mock/query_index_dir",
)
@patch(
    "aria_agents.chatbot_extensions.study_suggester.get_query_function",
    new_callable=lambda: MagicMock(return_value=mock_get_query_function()),
)
async def test_run_study_suggester(
    mock_get_query_index_dir,
    mock_get_query_function,
...
async def test_create_diagram(mock_artifact_manager, suggested_study, config):
    create_diagram = create_create_diagram_function(
        mock_artifact_manager, config["llm_model"]
    )
    mock_artifact_manager.put(suggested_study, "suggested_study.json", overwrite=True)
