    ...
    class OutputMessageHandler:
        ...
        def status_callback(self, message):
            ...
            if message["status"] == "finished":
                assert (
                    "error" not in self.output_messages[-1].lower()
                ), f"Chatbot mentioned error: {self.output_messages[-1]}"
...
def event_bus(output_handler):
    ...
    return_event_bus.on(
        "stream", lambda message: output_handler.status_callback(message.model_dump())
    )
...
def chat_input():
    return {
        ...
        "question2": "Generate a new study suggestion and experiment protocol based on the economic effects of the previous query",
        "chat_history": [],
...
def mock_artifact_manager(event_bus):
    ...
    mock.get_attachment = AsyncMock(
        side_effect=get_file_in_folder(
            "tests/assets/attachments", format_func=attachment_format
        )
    )
    mock.exists = AsyncMock(
        side_effect=lambda filename: os.path.exists(os.path.join(temp_dir, filename))
    )
